...
if(isset($_POST['add'])){   //check if form has been posted
  ...
  if(!$validateName){
    ...
    $newHub = $addEvent->addEvent($title, $date, $start, $end, $venue, $description, $logo);   //add event
    if($newHub){    //addition successful
      echo "<script> alert('NEW EVENT ADDED!');
      window.location.href='viewEvents.php';</script>";
    }else{
      echo "<script> alert('NEW EVENT UNSUCCESSFUL!');
            </script>";
    }
  }
